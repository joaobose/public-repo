...
def A_star_search(graph,goal,start):#encuentra el camino desde el nodo start hacia el goal(A* ALGORITHM)
    frontier = PriorityQueue()#cola que ordena sus elementos segun un coeficiente de "prioridad" (en orden decreciente, a menos coeficiente mayor prioridad)
    path = {}
    cost = {}

    frontier.put(vec2int(goal),0)#mi objetivo es por donde empiezo (agrego mi goal al frontier, con 0 de coeficiente (prioridad maxima))
    path[vec2int(goal)] = None
    cost[vec2int(goal)] = 0
    ...
    while not frontier.empty():#mientras el frontier no este vacio
        ...
        if current == start:#si ya llego a el start(ya encontro el camino)
            break#se rompe el ciclo

        for next in graph.find_neighbors(vec(current)):#itera por los vecinos del nodo actual
            next = vec2int(next)
            ...
            if next not in cost or next_cost < cost[next]:#si no he pasado por next o consegui una ruta mas economica que pasa por next
                ...
                priority = next_cost + heuristic(vec(next),start)#defino la prioridad de next en el frontier (costo + distancia heuristica)(se incluye la distancia, para que la prioridad la tome en cuenta y de esa forma de optmece el algoritmo (siempre va va a intentar recorrer hacia donde halla menos distancia hacia start))
                frontier.put(next,priority)#agrego "next" a mi frontier (tomando en cuenta el costo)
...
#for node in water:
#    G.weigth_tiles[node] = 5

#calculando el path desde el start al goal
goal = vec(5,6)
start = vec(7,4)
path = A_star_search(G,goal,start)
